...
15. Добавить второй прогон через «Из БД…» (как в п. 11), затем убрать первый прогон из сравнения — заголовок показывает только название оставшегося прогона, без хвоста «Модель · Эксперимент · Режим» (в том числе не показывает данные удалённого прогона).
