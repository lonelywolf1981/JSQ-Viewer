...
3. Включить «сред» — появляется горизонтальная линия с подписью вида «T8+ сред 4.7 (по 8 каналам)» у левого края, над линией, не налезая на кривую.
4. Включить «мин» и «макс» — добавляются точечная и штрих-пунктирная горизонтали того же цвета. В скобках назван канал, достигший экстремума, например «T8+ мин 4.3 (T14)». Это, как правило, НЕ тот канал, что выбран для показа на графике.
4a. Сузить диапазон так, чтобы экстремум пришёлся на другую термопару — имя канала в подписи изменилось соответственно.
...
10a. Снять галки со всех холодных термопар, оставив только тёплые — линия минимума всё равно видна: ось Y раздвигается вниз, чтобы её вместить. То же вверх для максимума.
10b. Задать границы оси Y вручную — заданные значения важнее, ось под уровни НЕ раздвигается, и уровень за их пределами не показывается. Это ожидаемое поведение.
...
12a. Подписи одной роли от разных источников не перекрываются: первая стоит слева, вторая справа — одинаково для минимума, среднего и максимума. Проверять на прогонах с близкими значениями, где линии идут почти вплотную.
