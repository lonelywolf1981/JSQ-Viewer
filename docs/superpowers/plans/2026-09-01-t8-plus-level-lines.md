...
Их читает `Presentation/WinForms/Charting/ChartViewModelFactory.cs:35-36`, поэтому там же удалить две строки:
...
вместе с запятой на предыдущей строке, если она остаётся висеть. Без этой правки проект не собирается до конца Task 3, а сборка обязана быть зелёной в конце каждой задачи. `ChartSeriesViewModel.Role` после этого остаётся значением по умолчанию `Channel` у всех серий, поэтому двухпроходная вставка в `ChartRenderer` кладёт всё в первый проход — ровно поведение до фичи. Сами свойства с `ChartSeriesViewModel` и мёртвые ветки рендерера убирает Task 3.
